...
function removeBook(id) {
  let cfm = confirm("Hapus Data Buku Ini?");
  ...
  if (cfm == true) {
    ...
    localStorage.setItem(localStorageKey, JSON.stringify(bookData));
    renderData(getData());
    alert(`Anda Yakin Akan Menghapus Buku ${bookDataDetail[0].title}?`);
  } else {
    return 0;
  }
  getBooksInformation();
...
function readedBook(id) {
  let cfm = confirm("Pindahkan Buku Ke Rak Sudah Dibaca ?");
...
function unreadedBook(id) {
  let cfm = confirm("Pindahkan Buku Ke Rak Belum Dibaca ?");
